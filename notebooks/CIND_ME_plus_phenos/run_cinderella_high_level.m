...
% Using the main CINDERellA() function with prior_matrix parameter

clear all; close all; clc;

fprintf('\n=== CINDERELLA WITH DIRECTIONAL PRIORS ===\n');
...
try
    ...
    % Load data using CINDERellA's read_exp function format
    % But we'll load it manually since we have a custom format
    data = readtable(input_file, 'FileType', 'text', 'Delimiter', '\t');
    
    fprintf('Data loaded: %d samples x %d variables\n', height(data), width(data)-1);
    
    % Extract data matrix and variable names
    % Data file format: rows=samples, cols=variables
    % CINDERellA needs: rows=variables, cols=samples
    data_matrix = table2array(data(:, 2:end))';  % TRANSPOSE to get variables as rows
    var_names = data.Properties.VariableNames(2:end);
    
    [nGenes, nSamples] = size(data_matrix);
    fprintf('Matrix transposed for CINDERellA: %d variables (genes) x %d samples\n', nGenes, nSamples);
    
    % VALIDATION CHECK
    if nGenes > 50
        error('ERROR: Too many variables (%d)! Expected ~9 variables. Data may be incorrectly formatted.', nGenes);
    end
    if nSamples < 50
        error('ERROR: Too few samples (%d)! Expected ~450 samples. Data may be incorrectly formatted.', nSamples);
    end
    
    fprintf('✓ Data format validated: %d variables, %d samples\n', nGenes, nSamples);
    ...
    % CONSTRAINT 4: Block Phenotype → Phenotype edges (they're both outcomes)
    for i = phenotype_indices
        for j = phenotype_indices
            if i ~= j
                prior_matrix(i, j) = 0;
    ...
    % Save prior matrix for CINDERellA to use
    prior_file = fullfile(output_dir, 'hprior.mat');
    Hprior.skeleton = sparse(prior_matrix);
    save(prior_file, 'Hprior');
    fprintf('Saved prior matrix to: %s\n', prior_file);
    ...
    if exist(edgefrq_file, 'file')
            ...
            for j = 1:nGenes
                if edgefrq_full(i, j) > 0.3  % Strong edges only
                    ...
                    elseif ismember(i, phenotype_indices) && ismember(j, module_indices)
                        ...
                        fprintf('  ⚠️  Phenotype → Module: %s → %s (%.3f)\n', ...
                            var_names{i}, var_names{j}, edgefrq_full(i, j));
        ...
        fprintf('\n=== DIRECTIONALITY ANALYSIS (edges > 0.3) ===\n');
